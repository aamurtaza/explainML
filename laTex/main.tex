...
%Task: It is not clear what alg 1 computes, explain it with much clarity that readers can implement.
%Explain alg 1. 
%\begin{enumerate}
%	\item alg 1 takes three inputs
%	\begin{enumerate}
%		\item testset instances
%		\item explanations of testset instances
%		\item epsilon in which neighborhood will be defined
%	\end{enumerate}
%	\item output array contains lipschitz estimate values of testing instances.
%	\item for every testing instance or vector in testset
%	\begin{enumerate}
%		\item compute neighborhood using epsilon using KDTree
%		\item compute equatons 1 and insert computed value in output array
%	\end{enumerate}
%	\item return output	
%\end{enumerate}
%
%Write down pre requistie steps and calculations performed in the actual implementation of alg 1. 
%\begin{enumerate}
%	\item Take dataset and prepare train and test sets
%	\item Fit predictive model on train set and evaluate it using test set.
%	\item Give predictive model and test set $X$ to explanation method to generate explanations $f(X)$
%	\item Take $\{x_{i},...,x_{n}\} \in X$, $\{f(x_{i}),...,f(x_{n})\} \in f(X)$, $\epsilon$ in algorithm 1 
%	\begin{enumerate}
%		\item For each anchor point x and define its neighborhood $N_{\epsilon}(x_{i})$ (write second algo)
%		\item For each anchor point x, its neighborhood $N_{\epsilon}(x_{i})$ compute equation 1. (write third algo)
%		\begin{enumerate}
%			\item Take anchor point x, its neigborhood points in $N_{\epsilon}(x_{i})$ and corresponding explanations to compute local lipschitz estimate values.
%			\item Pick that deviation point using argmax among estimated values
%			\item Return lipschitz estimate value of deviation point
%		\end{enumerate}
%	\end{enumerate}
%\end{enumerate}


%\subsubsection{Summary: alvarez2018robustness}
%\citet{alvarez2018robustness} proposes a robustness measure to evaluate local stability of explanations generated by explanation methods, mainly categorized into two approaches, saliency and perturbation. \citet{alvarez2018robustness} argue that explanations methods are sensitive to small variations in inputs and show that how slight change in inputs doesn't change the prediction but dramatically effects the explanations. Standard machine learning classifiers, UCI repository datasets and COMPAS dataset are used to conduct the experiments. To test the robustness of explanations of image-processing tasks, convolutional neural network based model trained on MNIST and ResNet classifier on ImageNet dataset are used.



%\subsubsection{Summary: alvarez2018towards}
